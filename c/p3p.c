...
LIBRARY_API int p3p_lambdatwist(Vector2 _y1, Vector2 _y2, Vector2 _y3, Vector3 x1, Vector3 x2, Vector3 x3,
...
LIBRARY_API void applyRT(Vector2 out, Matrix33 R, Vector3 T, Vector3 in) {
    Vector3 temp;
    apply3(temp, R, in);
    add3(temp, T, temp);
    out[0] = temp[0]/temp[2];
...
LIBRARY_API void toHomography(double* h, Matrix33 R, Vector3 T) {
    h[0] = R[0][0]/T[2];
    h[1] = R[0][1]/T[2];
    h[2] = T[0]/T[2];
    h[3] = R[1][0]/T[2];
...
LIBRARY_API void applyHomography(Vector2 out, double* h, Vector2 in) {
